...
from public.GRU_HCA_onebyone import OboHcaGruX, OboHcaGruH, OboHcaGru
from public.ZoneOut import OboZoneOutGru
from public.Global_Best import GlobalBest
from public.Valuate import fun_predict_auc_recall_map_ndcg
from public.Valuate import fun_save_best_and_losses, fun_save_atts
...
PATH_o10 = os.path.join(WHOLE, 'outbrain_users_10_100_0.5yi/')
PATH = PATH_t30
    ...
    def build_model_one_by_one(self, flag):
        ...
        if 0 == flag:        # Gru
            ...
            model = OboZoneOutGru(
                train=[self.tra_buys_masks, self.tra_masks, self.tra_buys_neg_masks],
                test =[self.tes_buys_masks, self.tes_masks, self.tes_buys_neg_masks],
                zoneout=[p['zoneout_cell'], p['zoneout_hidd']],
                alpha_lambda=[p['alpha'], p['lambda']],
                n_user=self.user_num,
                n_item=self.item_num,
                n_in=size,
            ...
            model = OboHcaGruX(
                ...
                n_in=size,
                n_hidden=size,
                window_input=p['window_x'],
                window_hidden=p['window_h'])
        elif 3 == flag:
            assert p['window_h'] >= 2
            model = OboHcaGruH(
                train=[self.tra_buys_masks, self.tra_masks, self.tra_buys_neg_masks],
            ...
            assert p['window_x'] >= 2 and p['window_h'] >= 2
            model = OboHcaGru(
                train=[self.tra_buys_masks, self.tra_masks, self.tra_buys_neg_masks],
                test =[self.tes_buys_masks, self.tes_masks, self.tes_buys_neg_masks],
                alpha_lambda=[p['alpha'], p['lambda']],
    ...
    for epoch in np.arange(p['epochs']):
        ...
        if 0 == epoch % 4 or epoch >= (p['epochs'] - 15):
            ...
            print('\tavg. time (train, user, test): %0.0fs,' % np.average(times0),
                  ...
                  '| model: {v1}, x{v2}_h{v3}'.format(v1=model_name, v2=p['window_x'], v3=p['window_h']))
        ...
        if epoch == p['epochs'] - 1:
            ...
            if model_name in ['OboHcaGru', 'HcaGru']:
                ...
                path = os.path.join(
                    os.path.split(__file__)[0], '..', 'Results_attention_weights', PATH.split('/')[-2])
                fun_save_atts(path, model_name, epoch, p, best, all_ats, os.path.join(PATH, p['dataset']))
            # 保存最优值、所有的损失值。
