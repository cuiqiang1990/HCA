...
# ======================================================================================================================
class OboHcaGruX(GruBasic):
    def __init__(self, train, test, alpha_lambda, n_user, n_item, n_in, n_hidden,
                 window_input, window_hidden):
        super(OboHcaGruX, self).__init__(train, test, alpha_lambda, n_user, n_item, n_in, n_hidden)
        rang = 0.5
    ...
    def __theano_train__(self, n_in, n_hidden):
        ...
        def recurrence(xp_t, xp_t1, xq_t1, h_t_pre1, cx):
            ...
            cx = T.concatenate((cx[1:], xp_t.reshape((1, n_in))))       # shape=(winx, 20)
            ...
            # loss
            upq_t = T.dot(h_t, xp_t1 - xq_t1)       # 正负样本训练。h(t) * (xp(t+1) - xq(t+1))
        ...
        seq_l2_sq = T.sum([T.sum(par ** 2) for par in [xps, xqs,
                                                       ui, wh, bi,
                                                       qx, rx, vc]])
        upq = T.sum(loss)
        seq_costs = (
...
# ======================================================================================================================
class OboHcaGruH(GruBasic):
    def __init__(self, train, test, alpha_lambda, n_user, n_item, n_in, n_hidden,
                 window_input, window_hidden):
        super(OboHcaGruH, self).__init__(train, test, alpha_lambda, n_user, n_item, n_in, n_hidden)
        rang = 0.5
        self.window_input = theano.shared(borrow=True, value=np.asarray(window_input, dtype='int32'))
        self.window_hidden = theano.shared(borrow=True, value=np.asarray(window_hidden, dtype='int32'))
        # context_h
    ...
    def __theano_train__(self, n_in, n_hidden):
        ...
        # 根据性质：T.dot((n, ), (n, ))得到scalar
            ...
            loss = log(1.0 + e^(-upq))
        """
        def recurrence(xp_t, xp_t1, xq_t1, h_t_pre1, ch):
            # gru_unit
            z_r = sigmoid(T.dot(ui[:2], xp_t) +
                          T.dot(wh[:2], h_t_pre1) + bi[:2])
            z, r = z_r[0], z_r[1]
            ...
            # loss
            upq_t = T.dot(hw, xp_t1 - xq_t1)    # 正负样本训练。h(t) * (xp(t+1) - xq(t+1))
            loss_t = T.log(sigmoid(upq_t))
            return [h_t, ch, loss_t]

        cumh = T.alloc(self.h0, winh, n_hidden)
        [_, _, loss], _ = theano.scan(          # h是h1~ht。loss是h0~ht-1和x1~xt计算得到的。
            fn=recurrence,
            sequences=[xps, xps[1:], xqs[1:]],
            outputs_info=[self.h0, cumh, None],
            n_steps=seq_length-1,
            truncate_gradient=-1)
        ...
        # cost, gradients, learning rate, l2 regularization
        lr, l2 = self.alpha_lambda[0], self.alpha_lambda[1]
        seq_l2_sq = T.sum([T.sum(par ** 2) for par in [xps, xqs, ui, wh, bi, qh, rh, e, f]])
        upq = T.sum(loss)
        seq_costs = (
            - upq +
            0.5 * l2 * seq_l2_sq)
...
# ======================================================================================================================
class OboHcaGru(GruBasic):
    def __init__(self, train, test, alpha_lambda, n_user, n_item, n_in, n_hidden,
                 window_input, window_hidden):
        super(OboHcaGru, self).__init__(train, test, alpha_lambda, n_user, n_item, n_in, n_hidden)
        rang = 0.5
        self.window_input = theano.shared(borrow=True, value=np.asarray(window_input, dtype='int32'))
        self.window_hidden = theano.shared(borrow=True, value=np.asarray(window_hidden, dtype='int32'))
        # context_x
    ...
    def __theano_train__(self, n_in, n_hidden):
        ...
        [_, _, _, loss], _ = theano.scan(               # h是h1~ht。loss是h0~ht-1和x1~xt计算得到的。
            fn=recurrence,
            sequences=[xps, xps[1:], xqs[1:]],
            outputs_info=[self.h0, cumx, cumh, None],
            n_steps=seq_length-1,
            truncate_gradient=-1)
