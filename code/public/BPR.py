...
class OboMf(MfBasic):
    ...
    def __theano_train__(self, ):
        ...
        l2_sqr = (
            T.sum([T.sum(par ** 2) for par in [ux, xp]]))
        costs = (
            loss +
            0.5 * l2 * l2_sqr)
        # SGD
        update_ux = T.set_subtensor(ux, ux - lr * T.grad(costs, ux))    # sub_tensor 求导更新
        ...
        self.train = theano.function(
            inputs=[uidx, pidx],
            outputs=loss,
            updates=[(self.ux, update_ux),     # 把 sub_tensor 的更新返回到原参数里
                     (self.lt, update_xp)])

    def train(self, u_idx, p_idx):
...
class OboVBpr(MfBasic):
    ...
    def __theano_train__(self):
        ...
        costs = (
            ...
            0.5 * l2 * bpr_l2_sqr +
            0.5 * l2_ev * bpr_l2_ev)
        pars_subs = [(self.ux, usr), (self.lt, xpq), (self.ue, use)]
        seq_updates = [(par, T.set_subtensor(sub, sub - lr * T.grad(costs, sub)))
                       for par, sub in pars_subs]
        pars_alls = [self.ei]
...
# 不用mini-batch了，速度太慢。
# 因为不像RNN那样，多条序列全部算完后用BPTT更新一次就行。
# BPR是每次购买都要更新，不然无法有效更新user vector，求导比较麻烦。
# ======================================================================================================================
class Bpr(MfBasic):
    def __init__(self, train, test, alpha_lambda, n_user, n_item, n_in, n_hidden):
    ...
    def __theano_train__(self):
        ...
        # pars_subs = [(self.ux, usrs), (self.lt, xpqs)]      # 不知道行不行，这里用xpqs……
        # seq_updates = [(par, T.set_subtensor(sub, sub - lr * T.grad(costs, sub)))
        #                for par, sub in pars_subs]
        # 以下是准确写法，但比one_by_one要慢。
        pars_subs = [(self.ux, usrs)]
        seq_updates = [(par, T.set_subtensor(sub, sub - lr * T.grad(costs, sub)))
                       for par, sub in pars_subs]
    ...
    def train(self, uidxs, pq_idxs_t, mask_t):
