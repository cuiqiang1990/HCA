...
class GlobalBest(object):
    ...
    def fun_obtain_best(self, epoch):
        ...
        total = (end - self.start).total_seconds() / 3600.0
        total = '%.2fh/%d=' % (total, epoch+1) + '%.2fh' % (total / (epoch+1))
        # 指标值、最佳的epoch
        a = one + '-----------------------------------------------------------------'
        b = one + 'All values is the "best * {v1}" on epoch {v2}: | {v3} | {v4}'\
            .format(v1=amp, v2=epoch, v3=end.strftime("%Y.%m.%d %H:%M:%S"), v4=total)
